...
struct UserData {
	...
	riscv_stdin_func_t stdin = nullptr;
	void *opaque = nullptr;
};
...
RISCVMachine *libriscv_new(const void *elf_prog, unsigned elf_length, RISCVOptions *options)
{
	...
	try {
		...
		u = new UserData {
			.error = options->error, .stdout = options->stdout, .stdin = options->stdin, .opaque = options->opaque
		};
		m->set_userdata(u);
		...
		m->set_stdin([] (auto& m, char* data, size_t size) {
			auto& userdata = (*m.template get_userdata<UserData> ());
			if (userdata.stdin)
				return userdata.stdin(userdata.opaque, data, size);
			else
				return 0l;
		});
