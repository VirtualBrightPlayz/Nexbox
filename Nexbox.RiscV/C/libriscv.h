...
typedef long (*riscv_stdin_func_t)(void *opaque, char *msg, unsigned size);
...
typedef struct {
	...
	riscv_stdin_func_t stdin; /* Stdout callback */
...
LIBRISCVAPI RISCVRegisters *libriscv_setup_preempt(RISCVMachine *m);
LIBRISCVAPI int libriscv_preempt(RISCVMachine *m, RISCVRegisters *regs, uint64_t address, uint64_t instruction_limit);
